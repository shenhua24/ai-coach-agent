...
def load_exercises(body_part=None, equipment=None, difficulty=None, query=None):
    ...
    data = api_get("/api/exercises", params)
...
def render_header():
    st.markdown(
        ...
        <div class="app-header">
            <h1>AI 健身教练</h1>
            <p>
            根据训练目标、经验等级、可用器械和当天反馈，生成一周或一个月训练周期。
            系统会把动作要点、训练部位、组数次数、休息时间和反馈后的调整放在同一个工作台里。
            </p>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_top_bar():
    ...
    if col2.button("退出登录", width="stretch"):
        try:
            ...
            pass
        for key in ["access_token", "user", "profile", "guidance", "training_plan", "feedback_records", "messages", "tool_calls", "exercises"]:
            st.session_state[key] = None if key in {"access_token", "user"} else [] if key in {"feedback_records", "messages", "tool_calls", "exercises"} else {}
        st.rerun()
    ...
    with st.form("profile_form"):
        ...
        with col1:
            ...
            sex = st.selectbox(
                "性别",
                sex_options,
                index=sex_options.index(profile.get("sex", "男")) if profile.get("sex") in sex_options else 0,
            )
        with col2:
        ...
        selected_equipment = st.multiselect(
            "当前拥有的器械",
            equipment_options,
            default=profile.get("available_equipment", ["徒手"]),
        )
        selected_parts = st.multiselect(
            "重点训练部位",
            body_parts,
            default=profile.get("target_body_parts", []),
        )
    ...
    if submitted:
        data = api_post(
            "/api/profile",
            {
                ...
                "available_equipment": selected_equipment or ["徒手"],
                "target_body_parts": selected_parts,
...
def render_plan_actions():
    ...
    with st.form("plan_form"):
        ...
        plan_goal = st.selectbox(
            "目标",
            reference.get("goals", ["塑形"]),
            index=reference.get("goals", ["塑形"]).index(profile.get("goal", "塑形")) if profile.get("goal") in reference.get("goals", []) else 0,
        )
        plan_parts = st.multiselect(
            "优先部位",
            reference.get("body_parts", []),
            default=profile.get("target_body_parts", []),
        )
        plan_equipment = st.multiselect(
            "可用器械",
            reference.get("equipment_options", []),
            default=profile.get("available_equipment", ["徒手"]),
        )
        col1, col2 = st.columns(2)
    ...
    payload = {
        "days_per_week": days,
        "goal": plan_goal,
        "focus_parts": plan_parts,
        "available_equipment": plan_equipment or ["徒手"],
        "experience_level": profile.get("experience_level", "初级"),
        "preferred_duration_min": profile.get("preferred_duration_min", 60),
    }
    ...
    if submitted:
        data = api_post(
            "/api/feedback",
            {
                ...
                "energy_score": energy,
                "soreness_parts": split_tags(soreness),
                "pain_parts": split_tags(pain),
                "sleep_quality": sleep,
                "note": note,
            },
        )
...
def render_chat():
    ...
    if prompt_cols[0].button("按我的器械做周计划", width="stretch"):
        send_message("根据我的画像和已有器械，生成一周训练计划。")
    if prompt_cols[1].button("解释一个动作", width="stretch"):
        send_message("告诉我杯式深蹲的训练要点、常见错误和注意事项。")
    if prompt_cols[2].button("根据反馈调整", width="stretch"):
        send_message("我今天 Day 1 完成了，但难度 9 分，腿部很酸，帮我记录并调整下一次训练。")
